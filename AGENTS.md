...
When creating _any_ validation tests or one-off logic scripts, default to `scripts/ad-hoc/` or `tests/e2e/` / `tests/integration/` (E2E-first); use `tests/unit/` only after listing isolation failure modes. Do not pollute the `/` root context.
...
6. Prefer an E2E/integration check; if isolating, list failure modes first then add a focused test (include the publicCreds shape assertion if you added a new embedded default)
...
5. Prefer E2E/integration; unit only after listing isolation failure modes
...
8. Prefer E2E/smoke + integration (`tests/integration/services/`, gated by `RUN_SERVICES_INT=1`); unit in `tests/unit/services/` only for listed failure modes. Update `docs/ops/RELEASE_CHECKLIST.md` smoke section.
...
**Test layer preference (HARD — E2E-first)**: Prefer E2E as the sole testing mechanism for complex features. Use integration next when multi-module/DB state must be proven without a full UI run. Unit tests are last-resort isolation checks for real failure modes (routing/admission/combo/auth/security) — never for UI chrome, formatters, i18n glue, docs sync, or snapshot noise. Encode bug reproductions as automated tests before or alongside the fix.

### Testing HARD rules (Ravi 2026-09-24)

1. **Never write unit tests after you write code.** If a unit test is justified, write the failure-mode list and the test *before* (or with) the implementation — never as post-hoc coverage filler.
2. **Highly prefer E2E tests as the sole testing mechanism.** Use them to verify complex features work end-to-end. At the end of E2E tests, produce a **verifiable and repeatable artifact** (e.g. saved response/trace/HAR/screenshot/log under `tests/e2e` fixtures or CI artifacts) so a human can re-check the same outcome.
3. **If you must test a system in isolation**, first write down all the ways it could fail, then write the code (and only then the focused unit/integration test that targets those failure modes). Do not invent tautological mocks that cannot catch a bug E2E would miss.

**Keep vs delete guidance for unit tests**: KEEP tests that prove routing, admission, combo failover/exhaustion, authz/route-guard, credential/proxy, and translation/protocol correctness. DELETE (do not add) trivial mocks, snapshot noise, tautologies, pure formatter/string/i18n glue, dashboard/UI chrome, gamification, and docs-sync checks that E2E or lint already cover.
...
18. Every bug fix must be validated before shipping: prefer a failing-then-passing **E2E** (with a verifiable repeatable artifact) or integration test; unit tests only for real isolation failure modes listed first. Otherwise a documented live test on the production VPS (192.168.0.15). A fix without either is not merged. Never write unit tests after the code. See Testing → "Testing HARD rules" and "Bug fix / issue triage protocol".
